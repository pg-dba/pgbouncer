...
PGPASSWORD=${DB_PASSWORD} psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d postgres -t -c "SELECT '\"' || usename || '\" \"' || passwd || '\"' FROM pg_shadow WHERE passwd is not null ORDER BY usename;" > /tmp/userlist.new.txt
